<?php
namespace Aequation\ToolwireBundle\Service;

// Aequation
use Aequation\ToolwireBundle\Service\interface\AppWireServiceInterface;
use Aequation\ToolwireBundle\Attribute\ClassCustomService;
// Aequation\ToolsBundle
use Aequation\ToolsBundle\tools\HttpRequest;
use Aequation\ToolsBundle\tools\Objects;
use Aequation\ToolsBundle\tools\Strings;
// Symphony
...
class AppWireService extends AppVariable implements AppWireServiceInterface
{
    ...
    public function getClassService(
        ...
        $attrs = Objects::getClassAttributes($objectOrClass, ClassCustomService::class);
        if(count($attrs)) {
            $attr = reset($attrs);
            $serviceName = $attr->service;
            if($this->hasService($serviceName)) return $this->getService($serviceName);
        }
        return null;
    }
    ...
    public function getTinyvalues(): array
    {
    ...
    // {
    //     return $this->getService(CacheServiceInterface::class);
    // }
    ...
    {
        return $this->getService('router')->getRouteCollection();
    }
    ...
    public function routeExists(string $route, bool|array $control_generation = false): bool
    {
        ...
        if($exists && $control_generation) {
            try {
                $this->getService('router')->generate($route, is_array($control_generation) ? $control_generation : []);
            } catch (\Throwable $th) {
                //throw $th;
                $exists = false;
            }
